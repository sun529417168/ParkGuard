...
import cn.com.watchman.service.TraceRePlay;
...
public class RecordShowActivity extends Activity implements
    ...
    private int mRecordItemId = -1, mRecordItemId2 = -1;
     ...
     * 轨迹数据初始化
     */
    private void setupRecord(int recordId) {
        // 轨迹纠偏初始化
        LBSTraceClient mTraceClient = new LBSTraceClient(getApplicationContext());
        DbAdapter dbhelper = new DbAdapter(this.getApplicationContext());
        dbhelper.open();
        PathRecord mRecord = dbhelper.queryRecordById(recordId);
        dbhelper.close();
        if (mRecord != null) {
